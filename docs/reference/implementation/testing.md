...
> Retained detailed implementation reference. Start with the consolidated
> [tests and acceptance plan](../../openbexi_timeline2.0_tests.md), which distinguishes
> earlier runtime evidence from the current documentation-only checkpoint.
...
```powershell
py -3 scripts/start.py --setup-only
npm test
...
On macOS/Linux use `python3` for setup and `.venv/bin/python` for the installed
project interpreter. These commands do not require a global uv installation.
...
.venv/Scripts/python.exe scripts/verify-candidate.py --matrix --output artifacts/verification/local-candidate-01
```
...
Each matrix entry now provisions its environment with `python scripts/start.py
--setup-only --python VERSION`, exercising the same installation path used by the
shared IntelliJ configuration on a clean checkout. This installs all Python groups
from `uv.lock`, npm dependencies from `package-lock.json`, and builds the client.
The launcher regression tests cover interpreter isolation, setup failures, client
dependency changes, and preserving the debugger when the IDE uses `.venv`.
