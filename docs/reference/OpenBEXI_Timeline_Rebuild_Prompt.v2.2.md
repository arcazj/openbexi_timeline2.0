...
Implementation specification and generation prompt | Revision 2.2 | 12 September 2026
...
Revision 2.2 restores the visual contract from the newly supplied 17-page `OpenBEXI_Timeline_Rebuild_Prompt_legacy.pdf`, adds eight legacy-aligned design screenshots, and incorporates three user-supplied screenshots as evidence. It strengthens collision-free track/text layout, compact menu continuity, right-side descriptors, filter-driven regrouping, yellow search highlights and findings-only overview behavior. Revision 2.1's dashboard-like visual direction is superseded; its model-authoring workflow requirements remain. Proposed screenshots are static design mockups, not implemented software or runtime test evidence.
...
**REQ-23 - A recognizable legacy operational workspace.** Open directly into populated timeline data or a clear empty workspace. Use the compact silver toolbar, centered report/reference time, horizontal time canvas, model-driven grouped bands, thin bars and point glyphs, light overview and right calendar/descriptor from the legacy brief. Do not replace this with a tall product header, permanent navigation sidebar or generic dashboard. Source/group/filter controls live in the compact strip, popovers and settings. Preserve multiple bands, overview navigation, grouping, nested activities, legends, point/session distinctions, original-time overlays, and source-backed cameras.
...
Resolve overlapping/dense records with deterministic reusable tracks and full rendered-footprint allocation under section 38, including complete text, icons, baseline graphics and focus/selection decorations. True temporal overlaps remain unchanged in the JSON records. Additional tracks, controlled label placement and scrollable band growth prevent unintended visual collisions; one permanent task row per event is not the default timeline model. Labeled overview density summaries at extreme zoom must offer access to their actual members. Provide empty, loading, malformed-data, disconnected, read-only, recovering, and permission-denied states that preserve useful content.
...
Release acceptance requires every mandatory requirement and A01-A56 to pass in the declared supported scope, plus the mapped fine-grained companion audit tests. Extend A22 to a multi-record cascade/batch and prove neither view briefly exposes a partial transaction. Unresolved mandatory failures mean the implementation is incomplete, even if a demonstration can run. Optional deferred features must remain clearly identified in documentation and must not appear as working controls.
...
| --- | --- |
| M0: audit and contracts | Complete source/feature inventory, listener/track/menu/search matrices, model coverage manifest, all 17 legacy-brief pages, runtime observations if possible, architecture decisions, storage protocol, schemas, OpenAPI and representative examples. Review eight legacy-aligned visual targets and three user evidence images. Record justified deviations; no unresolved conflict with JSON-only storage, prompt-only authorization or mandatory compatibility. |
...
The 17-page legacy brief governs the default Classic blue composition. Restore its metallic toolbar, compact icon order, centered report time, blue grouped bands, darker type gutter, thin colored bars, small glyphs, black reference marker, white right calendar/descriptor, gray overview and equivalent Table shell. Orthographic is the default. Do not reinstate revision 2.1's tall header/permanent left sidebar. Retain dark/neutral models and custom colors as first-class alternatives; the user screenshots establish the dark configuration and filter/group/search workflows, not a requirement to remove the blue default.
...
## 35. Complete brief and exact visual baseline
...
| Classic bands | type0 y=152..400; type4 y=400..604; type1 y=604..732. Alternating #A6D7F8 / #99BFD3; gutter #82A9BF. Customized band sizes grow/scroll with content. |
...
| A49 | Stress actual rendered footprints across long/unbroken/multiline labels, large fonts/icons, nested activities, model/group changes, pan/zoom/cameras, resize, selected/search styles and live edits. Confirm deterministic tracks, scrollable growth and no unintended intersection. |
